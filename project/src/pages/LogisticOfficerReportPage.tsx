...
const LogisticOfficerReportPage: React.FC = () => {
  const { requests, fetchRequests } = useRequestsStore();
  const { user, isAuthenticated } = useAuthStore();
  const navigate = useNavigate();

  useEffect(() => {
    if (!isAuthenticated || user?.role !== 'logistics-officer') {
      navigate('/');
    }
  ...
  // Filtered requests by type (logistics officer sees all requests)
