...
const AdminReportPage: React.FC = () => {
  console.log('UnitLeaderReportPage mounted');
  const { requests, fetchRequests, error } = useRequestsStore();
  const { user, isAuthenticated } = useAuthStore();
  const navigate = useNavigate();

  useEffect(() => {
    if (!isAuthenticated || user?.role !== 'unit-leader') {
      navigate('/');
    }
  }, [isAuthenticated, user, navigate]);

  useEffect(() => {
    // Only fetch requests if the user is authenticated and is a unit leader
    if (isAuthenticated && user?.role === 'unit-leader') {
  ...
  }, [fetchRequests, isAuthenticated, user]);
  ...
  // Filtered requests by type and by current user
  const filteredRequests = useMemo(() => {
    let base = requests.filter(r => r.type === reportType);
    // Only show requests made by the current unit leader
    if (user && user.id) {
      base = base.filter(r => {
    ...
    return base;
  }, [requests, reportType, status, range, customStart, customEnd, user]);
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      {error && <div className="text-red-600 font-bold mb-4">Error: {error}</div>}
